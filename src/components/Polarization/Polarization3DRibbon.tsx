...
interface Polarization3DRibbonProps {
  polarization: string;
  resize: number;
  socketStatus: string; 
  data: any;
...
const Polarization3DRibbon = ({ polarization, resize, socketStatus, data, legend }: Polarization3DRibbonProps) => {
  ...
  function getChartData(usedData: any, amplitude: boolean) {
    ...
    for (let i = 0; i < baseData.length; i++)
    {
        ...
        const tmpY = [];
        const tmpZ = [];
        for (let j = 0; j < baseData[i].data.length; j++)
        {
            tmpX.push([usedData.channels[j], usedData.channels[j]]);
            tmpY.push([baseData[i].data[j], baseData[i].data[j]]);
            tmpZ.push([i, i + 0.75]);
        }
      chartData.push(
        {
          ...
          colorscale: [[0, getLegendColor(baseData[i].name)],[1, getLegendColor(baseData[i].name)]], 
          type: 'surface',
          showscale: false
        }
      );
  ...
  return (
    <SignalCard
      title={`${t('label.polarization')  } 3D ${  polarization}`}
    ...
    >
      <Grid container direction="row" justifyContent="space-between">
        ...
        layout={{
            ...
            scene: {
              xaxis: {title: xLabel(), autorange: 'reversed' },
              yaxis: {visible: false },
              zaxis: {title: yLabel(true)}
            }
        }}/>
        </Grid>
        <Grid item xs={12}>
...
export default Polarization3DRibbon;
