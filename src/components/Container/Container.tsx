...
import Polarization3DRibbon from '../Polarization/Polarization3DRibbon';
import Spectrogram from '../Spectrogram/Spectrogram';
import SpectrumPlot from '../SpectrumPlot/SpectrumPlot';
import SubPlot from '../Polarization/SubPlot';
import Statistics from '../Statistics/Statistics';
import Socket from '../../services/WebSocket/Socket';
import PhaseData from '../../mockData/webSocket/phase.json';
import PlotData from '../../mockData/WebSocket/spectrum.json';
import { COLOR, DATA_LOCAL, MSG_PHASE_AMP, MSG_SPECTRUM, PROTOCOL, SOCKET_STATUS, WS_API_URL } from '../../utils/constants';
...
const Container = () => {
  ...
  function legendOnClick(val: string) {
    ...
    for (var i = 0; i < legendData.length; i++) {
      tmp.push({ 
        ...
        active: (legendData[i].text.toUpperCase() === val) ? !legendData[i].active : legendData[i].active
      });
    }
    setLegendData(tmp);
  }
  ...
  React.useEffect(() => {
    ...
    function getLegendData(usedData: any) {
      ...
      const elements = filtered.map((e, i) => { return {'text': e, 'active': true, color: COLOR[i] }}); 
      return elements;
    }
  ...
  return (
    <>
      ...
      <Legend resize={refresh} socketStatus={socketStatus1} data={legendData} onClick={legendOnClick} />
      <Polarization polarization='XX' resize={refresh} socketStatus={socketStatus1} data={chartData1} legend={legendData} />
      <SubPlot polarization='XX' resize={refresh} socketStatus={socketStatus1} data={chartData1} legend={legendData} />
      <Polarization3DRibbon polarization='XX' resize={refresh} socketStatus={socketStatus1} data={chartData1} legend={legendData} />
      <Polarization polarization='XY' resize={refresh} socketStatus={socketStatus1} data={chartData1} legend={legendData} />
      <Polarization polarization='YX' resize={refresh} socketStatus={socketStatus1} data={chartData1} legend={legendData} />
      <Polarization polarization='YY' resize={refresh} socketStatus={socketStatus1} data={chartData1} legend={legendData} />
      <Spectrogram />
    </>
  );
};
